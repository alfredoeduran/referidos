...
const menuItems = [
  ...
  { name: 'Gestión de Leads', icon: Users, href: '/admin/leads' },
  { name: 'Partners', icon: UserCheck, href: '/admin/partners' },
  { name: 'Comisiones', icon: DollarSign, href: '/admin/commissions' },
  { name: 'Descuentos', icon: Tag, href: '/admin/discounts' },
  { name: 'Roles', icon: Shield, href: '/admin/roles' },
]
      ...
      <nav className="lg:hidden fixed bottom-6 left-4 right-4 z-50">
        <div className="bg-[#2D2D2D]/95 backdrop-blur-md text-white rounded-3xl shadow-2xl border border-white/10 p-2 flex items-center justify-around">
          {menuItems.map((item) => {
            ...
            // For mobile, we might want shorter names or just icons for some
            const shortName = item.name === 'Gestión de Leads' ? 'Leads' : item.name
            ...
            return (
              <Link
                ...
                className={`flex flex-col items-center justify-center gap-1 p-2 rounded-2xl transition-all duration-300 min-w-[50px] ${
                  isActive 
                    ? 'text-[#F97316] bg-white/5 px-4' 
                    : 'text-gray-400'
                }`}
              >
                <item.icon className={`w-6 h-6 transition-transform ${isActive ? 'scale-110' : ''}`} />
                {isActive && (
                  <span className="text-[10px] font-bold uppercase tracking-tighter whitespace-nowrap animate-in fade-in slide-in-from-bottom-1 duration-300">
                    {shortName}
                  </span>
                )}
              </Link>
